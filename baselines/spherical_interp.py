"""Pure spherical interpolation (SLERP) baseline.

Directly SLERPs the source and target latent encodings and decodes through
the frozen FLUX reverse chain -- no per-pair optimization. This is the
"Spherical Interpolation" row in Table 1 of the paper.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Baseline: pure SLERP in latent space")
    parser.add_argument("source")
    parser.add_argument("target")
    parser.add_argument("--prompt", type=str, default="high quality image")
    parser.add_argument("--output-dir", type=str, default="./outputs/baseline_spherical")
    ...
    out = Path(args.output_dir) / datetime.datetime.now().strftime("spherical_%Y%m%d_%H%M%S")
    ...
    frames = []
    for i, alpha in enumerate(tqdm(alphas, desc="Spherical")):
        latents = slerp(z_src, z_tgt, float(alpha))
